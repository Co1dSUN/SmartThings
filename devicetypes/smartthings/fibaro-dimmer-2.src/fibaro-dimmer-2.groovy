 ...
 */
metadata {
	definition (name: "Fibaro Dimmer 2", namespace: "smartthings", author: "ColdSUN. Based on Rajiv, Elnar Hajiyev and Eric Maycock") {
...
def zwaveEvent(physicalgraph.zwave.commands.sceneactivationv1.SceneActivationSet cmd) {
    ...
    log.debug("Configuration for preference \"Switch Type\" is set to $settings.param20")
		...
		switch (cmd.sceneId) {
            ...
            break
            default:
                log.debug("Unhandled SceneActivationSet: ${cmd}")
            break
        
        
        }
   
/*    if (settings."param20" == "2") {
        log.debug("Switch configured as Roller blinds")
        switch (cmd.sceneId) {
            // Roller blinds S1
    ...
    } else if (settings."param20" == "1") {
        log.debug("Switch configured as Toggle")
        switch (cmd.sceneId) {
            // Toggle S1
            case 10: // Off to On
                buttonEvent(1, "pushed")
        ...
        if (settings."param20" == "0") log.debug("Switch configured as Momentary") else log.debug("Switch type not configured") 
        switch (cmd.sceneId) {
            // Momentary S1
            case 16: // 1x click
                buttonEvent(1, "pushed")
